...
class ProgramEfinixSRAMAppletAppletTestCase(GlasgowAppletV2TestCase, applet=ProgramEfinixSRAMApplet):
    # @synthesis_test
    def test_build(self):
        self.assertBuilds(args=["--creset", "A0", "--cs", "A1", "--cck", "A2", "--cdone", "A3", "--freset", "A4", "--cbus", "A5,A6,A7", "--cdi", "B0"])

    @applet_v2_simulation_test(args=["--creset", "A0", "--cs", "A1", "--cck", "A2", "--freset", "A4", "--cbus", "A5,A6,A7", "--cdi", "B0", "--frequency", "100"])
    async def test_iface(self, applet: ProgramEfinixSRAMApplet, ctx: SimulatorContext):
        from glasgow.gateware.cobs import encode
        await applet.efinix_iface.load(b'\x00\x01\xaa\x55\x80\xff')

    @applet_v2_simulation_test(args=["--creset", "A0", "--cs", "A1", "--cck", "A2", "--freset", "A4", "--cbus", "A5,A6,A7", "--cdi", "B0,B1,B2,B3,B4,B5,B6,B7", "--frequency", "100"])
